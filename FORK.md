...
> **2026-09-22**, onto `pingdotgg/t3code` `main` at **`76cc9b08`** — _chore(mobile): bump app
> version to 1.3.0_. Took in **81 upstream commits** (`8c3b5bef..76cc9b08`). The previous tip
> `7a32d882` is backed up at `origin/backup/main-pre-rebase-2026-09-22`; `main` was then
> force-pushed to the rebased tip.
>
> **Ten commits replayed, nothing superseded, nothing re-derived, one fallout edit.** Every
> drop-check in section 3 still comes back **0** against clean upstream. Three commits conflicted,
> in five files:
>
> - **Entry 14** — three modify/delete conflicts (`release.yml`, `mobile-fingerprint-check.yml`,
>   `.github/VOUCHED.td`), all deleted again. Separately, and with no conflict to flag it,
>   upstream's `e74c668d7` (#12519, _strip Alchemy progress before parsing relay state_) made
>   `scripts/release-smoke.ts` run a new `.github/scripts/relay-state-output.test.cjs` that reads
>   `release.yml` off disk; the fork's `release_smoke` job would have failed on the missing file.
>   Test deleted, six-line hook removed.
> - **Entry 15** — `63ff33756` (#12624) taught upstream's `resolveClaudeHomePath` the
>   `CLAUDE_CONFIG_DIR` → `~/.claude` precedence the fork's `resolveClaudeConfigDirPath` already
>   had, and its doc comment landed on the fork's credentials constant in `ClaudeHome.ts`. Both
>   helpers kept; the entry says when to collapse them.
> - **Entry 5** — `c14f6015b` (#12453, _keep narrow chat headers readable and aligned_) moved
>   `ChatHeader.tsx`'s action controls into a portaled `headerActions` element; the fork's
...
> each fork commit before and after the replay: identical for every file except the three entry 14
> deletions above, which now delete upstream's newer copies. The two upstream changes worth reading
> for their effect on a fork entry are the entry 15 and entry 5 ones above;
> `scripts/build-desktop-artifact.ts`, `release-desktop.yml` and `ci.yml` did not change this range,
> so `desktop-artifacts.yml` (entries 14 and 20) needed nothing.
>
> **Verification.** Node **24.13.1** from a tarball on `PATH` (the container ships 22). `pnpm
install --frozen-lockfile` clean. `tsc --noEmit` **0 errors** in contracts, shared,
> client-runtime, server, web, desktop and mobile. `vp lint` over the 66 fork-changed TypeScript
> files: **0 errors** (fifteen `set-state-in-effect` warnings, all on upstream lines of
> `ChatView.tsx`). `vp fmt --check` and `vp run knip:check` pass. `node scripts/release-smoke.ts`
> passes, and reproducibly failed on the missing `release.yml` before the entry 14 edit. The
> fork-touched test files pass: server usage, reactor, registry, probe and `ClaudeHome` suites
> (168), `server.test.ts` filtered to its worktree, bootstrap and prefix cases (17), and the shared,
> client-runtime and web unit files together (377). Nothing was checked in a browser.
>
...
- **Checked at `76cc9b08`: keep, one conflict resolved.** Drop-check empty. Upstream's `c14f6015b`
  ...
  button works in both presentations. `GitActionsControl.tsx` merged clean around the fork's hunks.
  `session-logic.ts` (`efb96939f` #7861), `ChatView.tsx` (`5781b5240` #12848, `e36725682` #12577),
  `ChangedFilesTree.tsx` (`f7efb5354` #12805) and `MessagesTimeline.tsx` (four commits) all took
  upstream lines away from the fork's hunks. The fork's added and removed lines are identical before
...
- **Checked at `76cc9b08`: keep, clean replay.** Drop-check **0**. `ThreadStatusIndicators.tsx` took
  `9f0c9f725` (#12545, reuse current PR status in the sidebar) away from the `hasUnseenCompletion`
  seam; `Sidebar.tsx` took `5781b5240` (#12848, undo settle/snooze/archive) and `f7efb5354`
  (#12805); `ChatView.tsx` took two commits away from the stamp. `uiStateStore.ts` and
  `Sidebar.logic.ts` are unchanged upstream this range.
...
- **Checked at `76cc9b08`: keep, clean replay.** `AGENTS.md` is unchanged upstream this range.
...
- **Checked at `76cc9b08`: keep, refreshed.** Rebase marker moved to `76cc9b08`; no entry moved this
...
- **Checked at `76cc9b08`: keep, three deletions re-applied, one new fallout edit.** Upstream's
  ...
  #12518), `mobile-fingerprint-check.yml` (`4931d73af` #12517), `.github/VOUCHED.td` (two vouches).
  `ci.yml`, `issue-labels.yml`, `thread-transfer-report.yml`, `release-desktop.yml`,
  `.github/scripts/thread-transfer-report.cjs` and `scripts/build-desktop-artifact.ts` are unchanged
  upstream this range, so `desktop-artifacts.yml` needed nothing. No new upstream workflow. `grep
-rn blacksmith .github/workflows/` still matches only the explanatory comment in
...
- **Checked at `76cc9b08`: keep, one conflict resolved.** Both server drop-checks still come back
  **0**. Upstream's `63ff33756` (#12624, _empty Claude homePath shares continuation with
  `~/.claude`_) gave `resolveClaudeHomePath` an optional `environment` and the same precedence the
  fork's `resolveClaudeConfigDirPath` implements — `homePath`, then an inherited
  `CLAUDE_CONFIG_DIR`, then `~/.claude` — so the continuation-group and capabilities-cache keys now
  agree with the `configDirectory` this entry reports. The only conflict was the two doc comments
  landing on the same lines at the top of `ClaudeHome.ts`. Both helpers are kept: upstream's has no
  `cwd`, and the fork's (upstream's own former skills-discovery helper) resolves a relative
  `CLAUDE_CONFIG_DIR` against the workspace. If upstream's grows a `cwd`, fold the fork's into it
  and this entry loses a function. `ClaudeProvider.ts`, `ClaudeSkills.ts`, `providerSnapshot.ts`,
  `providerStatusCache.ts`, `server.ts` and `ProviderInstanceCard.tsx` are unchanged upstream this
  range; `claudeSignedOutMessage` is still used only at turn time in `ClaudeAdapter.ts`.
...
- **Checked at `76cc9b08`: keep, clean replay.** Still the lone hardcoded const upstream. `ws.ts`
  took the device-management commits (`3b836f994` #12817, `61b25b88d` #12818, `1de563c14` #12877)
  away from the `prepareWorktree` block; `settings.ts` and `settingsSearch.ts` took `55c24273a`
  (#12779) around the fork's key and search entry; the reactor took `d6f291303` (#12625) and
  `server.test.ts` took `fa1e17155` (#12657) and `9cb586acd` (#9917), all clear of the fork's hunks.
  `git.ts` and the settings UI are unchanged upstream.
...
- **Checked at `76cc9b08`: keep, clean replay.** Upstream's `handleNewThreadClick` still never looks
  at the scope; its `scopedProjectGroup` hits are unchanged (scope combobox, search-empty state,
  member-set memo). `Sidebar.tsx` took `5781b5240` (#12848, undo) and `f7efb5354` (#12805) away from
  the fork's handler and props; `docs/user/thread-sidebar.md` took two undo paragraphs (`5781b5240`,
  `6b0a04ade` #10744) away from the fork's sentence. `Sidebar.logic.ts` and
  `SidebarThreadHeader.tsx` are unchanged upstream.
...
- **Checked at `76cc9b08`: keep, clean replay.** Drop-check **0**; `UsageProviderKind` is still
  `claude | codex | grok`. None of the entry's files — server, contracts, shared, web, mobile or any
  test fixture — changed upstream this range.
...
- **Checked at `76cc9b08`: keep, clean replay.** `scripts/build-desktop-artifact.ts` is unchanged
  upstream this range; `resolveGitHubPublishConfig` still reads `T3CODE_DESKTOP_UPDATE_REPOSITORY`
  first and still requires `owner/repo`. `SidebarChrome.tsx`, `branding.ts`, `vite.config.ts` and
...
- **Checked at `76cc9b08`: keep, clean replay.** Both drop-checks come back **0**.
  `ChatComposer.tsx` took five upstream commits (`6cc7f7006` #12837, `a4bc7deb9` #12836, `c789cd174`
  #12528, `933492470` #12719, `e36725682` #12577), all away from the entry's five hunks;
  `docs/user/composer.md` took `2b178231e` (#12620) away from the fork's section.
  `ModelPickerContent.tsx`, `ContextWindowMeter.tsx`, `SubscriptionUsageMeters.tsx` and the logic
  file are untouched upstream; `usedPercent` still crosses the wire.
...
| ------------ | --------------------------------------- | --------------------------------------------------------------------------------------------- | ----------- |
| 1            | Windows build: no shell mode            | `edb1240` — _fix(cli): publish nightly branded favicons (#4372)_                              | `76cc9b08`  |
| 4            | Terminal Ctrl-chord forwarding          | `acf761b2` — _feat(web): render terminals with libghostty-vt (#4860)_                         | `76cc9b08`  |
| 5 (core)     | Thread-scoped changed files             | `AssistantChangedFilesSection` per-turn checkpoints                                           | `76cc9b08`  |
| 7            | Shell-style composer recall             | `fd773172` — _feat(web): recall sent prompts with the up arrow (#9173)_                       | `76cc9b08`  |
| 8            | Full timestamp on hover                 | `formatChatTimestampTooltip` in `apps/web/src/timestampFormat.ts`                             | `76cc9b08`  |
| 9            | Always-visible new-thread btn           | `0de95407` — _feat: sidebar v2 is now the default sidebar (#5672)_                            | `76cc9b08`  |
| 10           | Package-local vitest configs            | `vp` (vite-plus) test-runner migration                                                        | `76cc9b08`  |
| 12 (symlink) | `CLAUDE.md` symlink → `AGENTS.md`       | `4cb676cc` — _docs: point CLAUDE.md at AGENTS.md with an @import (#7171)_                     | `76cc9b08`  |
| 15 (banner)  | Status banner prefers server msg        | `06336460` — _feat(providers): add Google Antigravity via the official ACP agent (#9348)_     | `76cc9b08`  |
| 16           | Usage scans every provider instance     | `2db675ae` — _fix(usage): respect provider account homes (#11485)_                            | `76cc9b08`  |
| 22 (server)  | Subscription usage collection           | `19d8ab2a` — _feat(usage): show Codex and Claude subscription limits on a Limits tab (#9507)_ | `76cc9b08`  |
| 18 (layout)  | Labelled new-thread btn under scope row | `d1d15c67` — _feat(sidebar): fold the project scope into the search row (#11315)_             | `76cc9b08`  |
| 21           | New project inside the scope menu       | `d1d15c67` — _feat(sidebar): fold the project scope into the search row (#11315)_             | `76cc9b08`  |
