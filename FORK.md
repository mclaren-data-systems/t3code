...
> **2026-09-12**, onto `pingdotgg/t3code` `main` at **`cf1ba3d7`** —
> _feat(settings): configure default permissions for new threads (#11346)_. Took in **80 upstream
> commits** (`b7b3ef1e..cf1ba3d7`). The previous tip `e72e9227` is backed up at
> `origin/backup/main-pre-rebase-2026-09-12`; `main` was then force-pushed to the rebased tip.
>
> **Straight replay, one commit per entry, no supersessions.** Every drop-check in section 3
> comes back **0** against clean upstream; nothing moved to section 4. The ten fork commits
> replayed in order and `git log upstream/main..main` still reads as section 3 does. Fix-ups
> found after the replay (the mobile markers below, a stray comment in entry 18, this file) were
> folded into their entry's commit with `--fixup` + `--autosquash`, not stacked.
>
> **Three entries conflicted, in six files.** Two were upstream reworks the fork had to follow,
> not adjacent-line noise:
>
> - **Entry 17 — upstream's per-project settings** (`2c0e89174` #11176, `8b2c0465d` #10636,
>   `e22040dfc` #10639). `settings.ts` conflicted beside the new `pullRequestMergeMethod` key
>   (both kept). `ProviderCommandReactor.ts` now reads settings through
>   `projectSettingsForThread` — the fork's prefix read follows it; for an environment-wide key
>   the value is the same. `SourceControlSettings.tsx` lost the `isPrimaryEnvironment` gate, so
>   `WorktreeBranchSettingsSection` was re-derived on the scoped hooks (details in entry 17).
>   The settings-search `primaryOnly` flag no longer exists upstream.
> - **Entry 19 — `48654c118` (#11021), unpriced model activity.** `usageMerge.ts`'s model
>   accumulator gained `unpricedRecords` on the same lines as the fork's `instanceId`/`model`
>   keys (both kept), and mobile's `UsageRouteScreen.tsx` model row took upstream's
>   "no known rates" / "Unpriced" rendering inside the fork's per-instance row.
>   **Trap for next time:** that second conflict scrolled past the visible tail of the rebase
>   output and a blanket `git add -A` committed its markers; only mobile `tsc` caught it. Run
>   `git diff --name-only --diff-filter=U` before staging any resolution.
> - **Entry 5 — `211618fd9` (#11169), huge-thread switch.** `ChatView.tsx` now paints a held
>   copy of the previous timeline while a large thread loads and nulls every interactive
>   `MessagesTimeline` prop for it; `onCommitTurnFiles` joined that gate.
>
> **Entry 14: nothing to do.** `.github/` is byte-identical between `b7b3ef1e` and `cf1ba3d7` —
> no workflow, action or script changed upstream — so `ci.yml`, `desktop-artifacts.yml` and the
> deletions replayed as-is and there was nothing to diff `desktop-artifacts.yml` against.
>
...
> client-runtime, web, server, desktop and mobile (filter on `error TS`; the Effect
> language-service `suggestion` lines are not errors). `vp lint` over the 66 fork-changed
> TypeScript files: **0 errors**, warnings only, all pre-existing patterns. `vp fmt --check`
> passes. `vp run knip:check` passes. The 18 fork-touched test files: **700 passed, 1 failed** —
> the `server.test.ts` case _"reports workspace root stat failures without relabeling them as
> missing"_, which `chmod 0o000`s a directory and expects the stat to fail; it cannot fail as
> uid 0 and the rebase container runs as root. Same environment artifact as every rebase since
> 2026-08-28, not a regression. Nothing was checked in a browser.
>
...
- **Checked at `cf1ba3d7`: keep.** Drop-check empty. Upstream's huge-thread switch
  (`211618fd9` #11169) paints a held copy of the previous timeline while a large thread loads
  and nulls every interactive `MessagesTimeline` prop for it; `onCommitTurnFiles` sits in the
  same gate (`paintOnlyDisplayedTimeline || !isServerThread ? undefined : …`). That one hunk
  conflicted; `GitActionsControl.tsx` (`50791a053` branch wording) and `MessagesTimeline.tsx`
  replayed clean.
...
- **Checked at `cf1ba3d7`: keep, clean replay.** Drop-check **0**. Upstream's PR-badge work
  (`0527ddf06`, `e784975bb`, `02297e3db`) touched `ThreadStatusIndicators.tsx` and `Sidebar.tsx`
  this range without conflicting; `uiStateStore.ts` is unchanged upstream.
...
- **Checked at `cf1ba3d7`: keep, clean replay.** Upstream did not touch `AGENTS.md` this range.
...
- **Checked at `cf1ba3d7`: keep, refreshed.** Rebase marker moved to `cf1ba3d7`; the banner text
  itself is unchanged since no entry moved.
...
- **Checked at `cf1ba3d7`: keep; `.github/` untouched upstream this range.** No workflow,
  action or script changed between `b7b3ef1e` and `cf1ba3d7`, so nothing was re-derived and
  `grep -rn blacksmith .github/workflows/` still matches only the explanatory comment in
...
- **Checked at `cf1ba3d7`: keep, clean replay.** Both server drop-checks still come back **0**.
  Upstream's shared-settings-row alignment (`4d16e6bc6` #10571) rewrote 196 lines of
  `ProviderInstanceCard.tsx` around the fork's "Resolved config directory" row without
  conflicting; `ClaudeProvider.ts`, `ClaudeHome.ts` and `providerSnapshot.ts` are unchanged
  upstream this range.
...
- **Checked at `cf1ba3d7`: keep, clean replay.** Drop-check **0**. `UsageService.ts` and
  `ProviderInstanceRegistryHydration.ts` (home of `deriveProviderInstanceConfigMap`) are unchanged
  upstream this range, so the duplicated merge rule has not drifted.
...
- **Checked at `cf1ba3d7`: keep, web side re-derived.** Still the lone hardcoded const upstream.
  Upstream's per-project settings (`2c0e89174` #11176, `8b2c0465d` #10636, `e22040dfc` #10639)
  conflicted in three files. `settings.ts`: adjacent to the new `pullRequestMergeMethod`, both
  kept. `ProviderCommandReactor.ts`: the settings read is now `projectSettingsForThread`, which
  returns the environment settings with the thread's project overrides applied — identical for
  an environment-wide key — and the fork's guard sits under it unchanged. `SourceControlSettings.tsx`:
  upstream removed the `isPrimaryEnvironment` gate and the `usePrimarySettings` pattern, so
  `WorktreeBranchSettingsSection` now uses `useScopedSettings` / `useUpdateScopedSettings` /
  ...
  `scope: "environment-defaults"` because upstream deleted the `primaryOnly` flag.
...
- **Checked at `cf1ba3d7`: keep, clean replay.** Upstream changed 56 lines of `Sidebar.tsx`
  this range (PR badges, link icon, remote-project markers in pickers) without reaching the header
  block. A stray comment about settled-thread partitioning that the previous rebase had left in
  this commit was removed; the diff is the header re-stack and nothing else. Still the fork's
...
- **Checked at `cf1ba3d7`: keep.** Drop-check **0**; `UsageProviderKind` is still
  `claude | codex | grok`. Upstream's `48654c118` (#11021) added `unpricedRecords` to
  `ModelTotals` and `isModelCostUnknown`; it conflicted in `usageMerge.ts` (model accumulator —
  the fork's `instanceId`/`model` keys beside upstream's `unpricedRecords`, both kept, and the
  merged `ModelTotals` carries both) and in mobile's `UsageRouteScreen.tsx`, whose model row now
  shows upstream's "no known rates" / "Unpriced" inside the fork's per-instance layout. Web's
  `UsagePage.tsx` auto-merged with the same treatment. Its new `UsagePage.test.tsx` fixtures
  typecheck clean against the instance fields.
...
- **Checked at `cf1ba3d7`: keep, clean replay.** `resolveGitHubPublishConfig` and its
  `T3CODE_DESKTOP_UPDATE_REPOSITORY` read are unchanged upstream, so the disable holds.
...
- **Checked at `cf1ba3d7`: keep, clean replay.** Upstream still renders the standalone
  folder-plus button next to the scope trigger and no in-menu item.
...
- **Checked at `cf1ba3d7`: keep, clean replay.** Both drop-checks come back **0**. `usedPercent`
  still crosses the wire. `ChatComposer.tsx` took two upstream commits this range (`ef6fa1187`
  draft preservation during compaction, `cf1ba3d7d` default permissions) without touching the
  five hunks.
...
| ------------ | --------------------------------- | --------------------------------------------------------------------------------------------- | ----------- |
| 1            | Windows build: no shell mode      | `edb1240` — _fix(cli): publish nightly branded favicons (#4372)_                              | `cf1ba3d7`  |
| 4            | Terminal Ctrl-chord forwarding    | `acf761b2` — _feat(web): render terminals with libghostty-vt (#4860)_                         | `cf1ba3d7`  |
| 5 (core)     | Thread-scoped changed files       | `AssistantChangedFilesSection` per-turn checkpoints                                           | `cf1ba3d7`  |
| 7            | Shell-style composer recall       | `fd773172` — _feat(web): recall sent prompts with the up arrow (#9173)_                       | `cf1ba3d7`  |
| 8            | Full timestamp on hover           | `formatChatTimestampTooltip` in `apps/web/src/timestampFormat.ts`                             | `cf1ba3d7`  |
| 9            | Always-visible new-thread btn     | `0de95407` — _feat: sidebar v2 is now the default sidebar (#5672)_                            | `cf1ba3d7`  |
| 10           | Package-local vitest configs      | `vp` (vite-plus) test-runner migration                                                        | `cf1ba3d7`  |
| 12 (symlink) | `CLAUDE.md` symlink → `AGENTS.md` | `4cb676cc` — _docs: point CLAUDE.md at AGENTS.md with an @import (#7171)_                     | `cf1ba3d7`  |
| 15 (banner)  | Status banner prefers server msg  | `06336460` — _feat(providers): add Google Antigravity via the official ACP agent (#9348)_     | `cf1ba3d7`  |
| 22 (server)  | Subscription usage collection     | `19d8ab2a` — _feat(usage): show Codex and Claude subscription limits on a Limits tab (#9507)_ | `cf1ba3d7`  |
