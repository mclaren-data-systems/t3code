...
(5, 6, 18). Everything else is byte-identical to upstream — `native/`, `scripts/` (entry 14's deletion of one
orphaned release script and one six-line hunk in `release-smoke.ts` aside), `apps/desktop/`,
`packages/client-runtime/` (one test fixture aside), `apps/server/src/persistence/`,
...
> **2026-09-25**, onto `pingdotgg/t3code` `main` at **`d4a33457`** — _fix(desktop): desktop
> updates reconnect in seconds, not minutes (#12006)_. Took in **203 upstream commits**
> (`8c3b5bef..d4a33457`). The previous tip `980f525f` is backed up at
> `origin/backup/main-pre-rebase-2026-09-25`; `main` was then force-pushed to the rebased tip.
> The replay went in two stages: the ten fork commits were first taken from the unpublished
> `rebase/main-2026-09-22` branch (the same commits already replayed onto `76cc9b08`, 81 commits
> in, and verified there) and then rebased over the remaining 122.
>
> **Ten commits replayed, nothing superseded, nothing re-derived; two fixture edits and seven
> lint repairs.** Every drop-check in section 3 still comes back **0** against clean upstream.
> Six commits conflicted across the two stages, all resolved mechanically:
>
> - **Entry 14** — seven modify/delete conflicts over the two stages (`release.yml` three times,
>   `mobile-fingerprint-check.yml`, `mobile-eas-preview.yml`, `mobile-showcase-screenshots.yml`,
>   `.github/VOUCHED.td` twice), all deleted again. One fallout edit with no conflict to flag it:
>   `e74c668d7` (#12519) made `scripts/release-smoke.ts` run a new
>   `.github/scripts/relay-state-output.test.cjs` that reads `release.yml` off disk, which the
>   fork's `release_smoke` job would have failed on. Test deleted, six-line hook removed. The
>   `pnpm-workspace.yaml` placeholder repair from the old `2baebf1c` is gone: upstream's
>   `803f94e78` (#12544) deleted the line itself, so the file is upstream's again.
> - **Entry 15** — `63ff33756` (#12624) put a doc comment on the same `ClaudeHome.ts` lines as
>   the fork's credentials constant; both kept.
> - **Entry 5** — `c14f6015b` (#12453) portaled `ChatHeader.tsx`'s action controls; the fork's
>   `commitPreselection` prop was re-added on the new `GitActionsControl` call site.
> - **Entry 17** — one import line in `ws.ts`, beside `0141bc2bf`'s (#12955)
>   `resolveProjectSettings` import.
> - **Entry 19** — `b954af60c` (#13104) deleted `UsagePage.test.tsx`, which the fork had only
>   adapted fixtures in: deletion taken. In `UsagePage.tsx` its `sortModelsByTokens` import and
>   `f26ee083f`'s `text-2xs` landed on the fork's lines; both kept.
> - **Entry 20** — one import line in `SidebarChrome.tsx`.
>
> Every other entry replayed clean — verified per file by comparing the added and removed lines of
> each fork commit before and after the replay: identical everywhere except the entry 14
> deletions above, which now delete upstream's newer copies, and the deleted usage test. Four
> upstream changes then bit after the replay, each fixed inside the entry's own commit:
>
> - Two new upstream test fixtures needed entry fields — `usageBreakdown.test.ts` (`b954af60c`)
>   builds a `ModelTotals` without entry 19's `instanceId`, and the banked-resets test in
>   `ProviderRegistry.test.ts` (`3412097bf` #13118) builds a probe result without entry 15's
>   `apiKeySource`. Typecheck, not merge, is what catches these.
> - Upstream's new lint rules fail on fork lines: `shadcn/no-restyle` (`5a61f50cc` #12982,
>   made an error by `10882bea7` #13210), `no-arbitrary-values` (`f26ee083f` #13397) and
>   `no-raw-colors` (`315fcca10` #13371). Seven errors, all fixed in place: entry 22's
>   `text-[11px]` / `text-[10px]` are now `text-2xs` / `text-3xs`; entry 15's config-directory
>   `ScrollArea` lost its shape, colour and padding classes to a wrapping `div`; entry 20's
>   GitHub icon is `text-destructive!` instead of `text-red-500!`. The `check` job runs lint on
>   the whole tree, so a class a fork line uses can be outlawed by a commit that never touches
>   the file — run `vp lint` over the fork-changed files after every rebase.
>
> `scripts/build-desktop-artifact.ts`, `release-desktop.yml` and `ci.yml` are unchanged this
> range (two lint commits added and then removed a three-line ceiling gate in `ci.yml`, net
> zero), so `desktop-artifacts.yml` (entries 14 and 20) needed nothing.
>
...
> client-runtime, server, web, desktop and mobile. `vp lint` over the 65 fork-changed TypeScript
> files: **0 errors** after the seven repairs above (210 warnings, every one also reported on
> clean upstream's copy of `ChatView.tsx` and friends). `vp fmt --check` over the fork-changed
> files and `vp run knip:check` pass. `node scripts/release-smoke.ts` passes with `vp` on
> `PATH`. The fork-touched test files pass: server usage, reactor, registry, probe and
> `ClaudeHome` suites (231), `server.test.ts` filtered to its worktree, bootstrap and prefix
> cases (17), and the shared, client-runtime and web unit files together (370). Nothing was
...
- **Checked at `d4a33457`: keep, one conflict resolved.** Drop-check empty. Upstream's `c14f6015b`
  (#12453, _keep narrow chat headers readable and aligned_) rewrote the action row in
  `ChatHeader.tsx`: the three controls now live in one `headerActions` element portaled either into
  an inline toolbar or into an overflow menu, and `GitActionsControl` gained a `presentation` prop.
  The fork's `commitPreselection={commitPreselection}` conflicted at the old inline call site and
  was re-added on the portaled one. Nothing else needed changing: the preselection effect opens the
  dialog through state, and the commit `Dialog` renders outside the toolbar/menu branch, so the
  button works in both presentations. After that, the ui-restyle series (`71c9b6931` #13208,
  `f46522777` #13207, `361b047f5` #13193 and neighbours) and `66129c6fd` (#13060, shell commands
  from chat) reworked `GitActionsControl.tsx`, `ChatView.tsx`, `MessagesTimeline.tsx`,
  `ChangedFilesTree.tsx` and `ChatHeader.tsx` around the fork's hunks without touching them; the
  fork's added and removed lines are identical before and after the replay, and none of them
  trips the new lint rules.
...
- **Checked at `d4a33457`: keep, clean replay.** Drop-check **0**. `ThreadStatusIndicators.tsx`
  took `9f0c9f725` (#12545, reuse current PR status in the sidebar) and three restyle commits away
  from the `hasUnseenCompletion` seam; `Sidebar.tsx` took ~20 commits (undo, auto-settle
  `010967041` #11846, the restyle series) clear of the fork's props; `ChatView.tsx` took two
  commits away from the stamp. `Sidebar.logic.ts` lost upstream's own `resolveThreadRowClassName`
  (`80fc23ac3` #13366), nothing of the fork's. `uiStateStore.ts` is unchanged upstream.
...
- **Checked at `d4a33457`: keep, clean replay.** `AGENTS.md` gained one upstream bullet (the
  `components/ui` no-restyle rule, `5a61f50cc` #12982 / `10882bea7` #13210) in its Taste section,
  well below the fork's two sections.
...
- **Checked at `d4a33457`: keep, refreshed.** Rebase marker moved to `d4a33457`; no entry moved this
  rebase, so the banner text is otherwise unchanged. Upstream did not touch `README.md` this range.
...
- **Files:** `.github/workflows/{ci,desktop-artifacts}.yml`; deleted
  ...
  `release-desktop.yml` ran it, and `knip:check` fails on an orphaned script); deleted
  `.github/scripts/relay-state-output.test.cjs` and the six-line hook in `scripts/release-smoke.ts`
  that ran it (it reads the deleted `release.yml` off disk); fork notes in
...
- **Re-apply.** Highest-churn entry. Re-derive from upstream's **new** workflow files and re-apply
  ...
  **without ever showing up as a merge conflict** — diff it against upstream's
  ...
  `scripts/release-smoke.ts` is the other silent-drift point: the fork's `release_smoke` job runs
  it, upstream uses it to exercise release-only steps, and a test it spawns may read a workflow
  the fork deletes — run it locally after every rebase.
- **Checked at `d4a33457`: keep, seven deletions re-applied, one fallout edit.** Upstream's
  `e74c668d7` (#12519, _strip Alchemy progress before parsing relay state_) added
  `.github/scripts/relay-state-output.test.cjs`, which reads `release.yml` off disk to exercise its
  relay-state step, and made `scripts/release-smoke.ts` run it under `node --test` before its other
  checks. The fork deletes `release.yml` and its `release_smoke` job runs that script, so the test
  dies on the missing file (reproduced: `ENOENT … release.yml`). Deleted the test and removed the
  six-line hook — the same shape as the former `deploy.test.ts` fallout — and `release-smoke.ts`
  passes again. Modify/delete conflicts, all deleted again: `release.yml` (`e74c668d7`, `bd0b9edac`
  #12518, `e407f9bb0` #13321 sharded release tests), `mobile-fingerprint-check.yml` (`4931d73af`
  #12517), `mobile-eas-preview.yml` (`68fb7f4b8` #13302), `mobile-showcase-screenshots.yml`
  (`11e91f126` #13316), `.github/VOUCHED.td` (three vouches). `ci.yml` is net-unchanged
  (`5a61f50cc` #12982 added a three-line no-restyle ceiling gate and `10882bea7` #13210 removed
  it), and `issue-labels.yml`, `thread-transfer-report.yml`, `release-desktop.yml`,
  `.github/scripts/thread-transfer-report.cjs` and `scripts/build-desktop-artifact.ts` are
  unchanged upstream this range, so `desktop-artifacts.yml` needed nothing. No new upstream
  workflow. `grep -rn blacksmith .github/workflows/` still matches only the explanatory comment in
  `desktop-artifacts.yml`. The `pnpm-workspace.yaml` placeholder the fork briefly deleted
  (`2baebf1c`) is gone upstream too (`803f94e78` #12544); the file is upstream's again.
...
- **Checked at `d4a33457`: keep, one conflict resolved, two repairs.** Both server drop-checks
  still come back **0**. Upstream's `63ff33756` (#12624, _empty Claude homePath shares
  continuation with `~/.claude`_) gave `resolveClaudeHomePath` an optional `environment` and the
  same precedence the fork's `resolveClaudeConfigDirPath` implements — `homePath`, then an
  inherited `CLAUDE_CONFIG_DIR`, then `~/.claude` — so the continuation-group and
  capabilities-cache keys now agree with the `configDirectory` this entry reports. The only
  conflict was the two doc comments landing on the same lines at the top of `ClaudeHome.ts`. Both
  helpers are kept: upstream's has no `cwd`, and the fork's (upstream's own former
  skills-discovery helper) resolves a relative `CLAUDE_CONFIG_DIR` against the workspace. If
  upstream's grows a `cwd`, fold the fork's into it and this entry loses a function. Later,
  `3412097bf` (#13118, banked resets) added a `resolveResetCredits` parameter after the fork's
  in `checkClaudeProviderStatus` and a test whose probe fixture lacked the fork's `apiKeySource`
  field — added, one line. `96c4bfa0a` (#13130) and `7e65b226e` (#12983, shared sign-in flows)
  changed `server.ts` and `ProviderInstanceCard.tsx` away from the fork's hunks; the card's
  config-directory `ScrollArea` then tripped upstream's new `no-restyle` rule and now sits inside
  a plain `div` that carries the shape, colour and padding. `claudeSignedOutMessage` is still
  used only at turn time in `ClaudeAdapter.ts`.
...
- **Re-apply.** Five decisions worth keeping:
  1. **The server owns the naming, the clients stay dumb.** Web and mobile mint the placeholder
     ...
     unchanged and the client call sites are untouched (four since `0150c6a5` #12179 — the web
     multi-model flow mints one placeholder per worktree — and every one of them lands in the
     same `ws.ts` bootstrap block, so the rewrite covers a call site upstream adds without a
     fork change). `ws.ts` rewrites the placeholder at
...
- **Checked at `d4a33457`: keep, one conflict resolved.** Still the lone hardcoded const upstream.
  The conflict was one import line in `ws.ts`, where `0141bc2bf` (#12955, submodule init for new
  worktrees) added `resolveProjectSettings` beside the fork's `applyWorktreeBranchPrefix`; the
  same commit inserted a `resolveBootstrapWorktreeSubmodules` call between the fork's rewrite and
  `createWorktree`, which is fine — the rewrite only needs to precede the create. `settings.ts`,
  `settingsSearch.ts` and the reactor took the same submodule commits around the fork's key,
  search entry and rename site; `git.ts` gained `sanitizeNewRefName` (`84c436bcf` #13454) below
  the prefix code; `server.test.ts` took `e67abcf79` (#13355) and `f193a6863` (#13109).
  `SourceControlSettings.tsx` took restyle commits; `WorktreeBranchSettings.tsx` is fork-only and
  passes the new lint rules.
...
- **Checked at `d4a33457`: keep, clean replay.** Upstream's `handleNewThreadClick` still never
  looks at the scope; its `scopedProjectGroup` hits are unchanged (scope combobox, search-empty
  state, member-set memo). `Sidebar.tsx` took the undo, auto-settle and restyle commits away from
  the fork's handler and props; `SidebarThreadHeader.tsx` took `be80e3693` (#13028, `SidebarInput`)
  and three restyle commits around the fork's optional prop; `docs/user/thread-sidebar.md` took
  the undo and auto-settle paragraphs away from the fork's sentence. `Sidebar.logic.ts` changed
  only for `80fc23ac3` (#13366), away from the fork's helper.
...
- **Re-apply.** `UsageBucket` gains `instanceId`; `UsageSource` gains `instanceId`, `displayName`,
  ...
  **Every upstream test fixture that builds a `UsageBucket`, `UsageSource` or `ModelTotals` needs
  the new fields** — upstream keeps adding usage tests (`apps/web/src/state/usage.test.tsx`,
  `packages/client-runtime/src/state/serverUsage.test.ts`, and at `d4a33457`
  `apps/web/src/components/usage/usageBreakdown.test.ts`) and they fail typecheck, not merge. Run
  the typecheck before assuming a clean merge is a clean rebase.
...
- **Checked at `d4a33457`: keep, one conflict resolved, one fixture added.** Drop-check **0**;
  `UsageProviderKind` is still `claude | codex | grok`. `b954af60c` (#13104, _cover usage model
  ordering without static markup_) deleted `UsagePage.test.tsx` — the fork had only adapted its
  fixtures, so the deletion is taken — added `sortModelsByTokens` to `UsagePage.tsx` on the
  fork's import line, and added `usageBreakdown.test.ts`, whose `ModelTotals` fixture needed an
  `instanceId`. Its new `usageMerge.test.ts` case builds buckets through the fork's helper and
  needed nothing. `f26ee083f` (#13397) changed a `text-[11px]` to `text-2xs` on a line the fork
  rewrites in `UsagePage.tsx`; kept upstream's class with the fork's `entry.label`. The other web
  usage components and mobile's `UsageRouteScreen.tsx` took restyle and platform-variant commits
  (`db898a306` #13188) away from the fork's hunks. Server, contracts and shared sources are
  unchanged upstream this range.
...
- **Re-apply.** Two halves:
  ...
  2. **Sidebar GitHub link.** A utility item in the sidebar footer right of Usage, same
     ...
     the theme's destructive red (`text-destructive!` **needs** the important marker —
     `SidebarMenuButton` forces `[&>svg]:text-[var(--sidebar-icon-color)]`; a raw palette class
     such as `text-red-500` fails upstream's `no-raw-colors` lint). It is a real anchor with `target="_blank"`, which
...
- **Checked at `d4a33457`: keep, one conflict resolved, one repair.** `scripts/build-desktop-artifact.ts`
  is unchanged upstream this range; `resolveGitHubPublishConfig` still reads
  `T3CODE_DESKTOP_UPDATE_REPOSITORY` first and still requires `owner/repo`. `SidebarChrome.tsx`
  took six commits (`9a91177c3` #13516 Back always returns to the main app, `9a609a4e4` #12972
  undo notice, restyles); the only conflict was the import line, and the fork's item still sits
  right of Usage inside the same conditional block. `315fcca10` (#13371) outlawed raw palette
  colours, so the icon is now `text-destructive!`. `branding.ts`, `vite.config.ts` and
  `vite-env.d.ts` are unchanged upstream.
...
- **Checked at `d4a33457`: keep, clean replay, one repair.** Both drop-checks come back **0**.
  `ChatComposer.tsx` took fifteen upstream commits (composer controls owning their look,
  `0e512db34` #13209, and the rest of the restyle series), `ModelPickerContent.tsx` three and
  `ContextWindowMeter.tsx` five, all away from the entry's hunks; `docs/user/composer.md` took
  `2b178231e` (#12620) and `66129c6fd` (#13060) away from the fork's section. The fork-only
  `SubscriptionUsageMeters.tsx` tripped `no-arbitrary-values` (`f26ee083f` #13397): its
  `text-[11px]` and `text-[10px]` are now `text-2xs` and `text-3xs`, the same pixel sizes on the
  scale. `usedPercent` still crosses the wire.
...
| ------------ | --------------------------------------- | --------------------------------------------------------------------------------------------- | ----------- |
| 1            | Windows build: no shell mode            | `edb1240` — _fix(cli): publish nightly branded favicons (#4372)_                              | `d4a33457`  |
| 4            | Terminal Ctrl-chord forwarding          | `acf761b2` — _feat(web): render terminals with libghostty-vt (#4860)_                         | `d4a33457`  |
| 5 (core)     | Thread-scoped changed files             | `AssistantChangedFilesSection` per-turn checkpoints                                           | `d4a33457`  |
| 7            | Shell-style composer recall             | `fd773172` — _feat(web): recall sent prompts with the up arrow (#9173)_                       | `d4a33457`  |
| 8            | Full timestamp on hover                 | `formatChatTimestampTooltip` in `apps/web/src/timestampFormat.ts`                             | `d4a33457`  |
| 9            | Always-visible new-thread btn           | `0de95407` — _feat: sidebar v2 is now the default sidebar (#5672)_                            | `d4a33457`  |
| 10           | Package-local vitest configs            | `vp` (vite-plus) test-runner migration                                                        | `d4a33457`  |
| 12 (symlink) | `CLAUDE.md` symlink → `AGENTS.md`       | `4cb676cc` — _docs: point CLAUDE.md at AGENTS.md with an @import (#7171)_                     | `d4a33457`  |
| 15 (banner)  | Status banner prefers server msg        | `06336460` — _feat(providers): add Google Antigravity via the official ACP agent (#9348)_     | `d4a33457`  |
| 16           | Usage scans every provider instance     | `2db675ae` — _fix(usage): respect provider account homes (#11485)_                            | `d4a33457`  |
| 22 (server)  | Subscription usage collection           | `19d8ab2a` — _feat(usage): show Codex and Claude subscription limits on a Limits tab (#9507)_ | `d4a33457`  |
| 18 (layout)  | Labelled new-thread btn under scope row | `d1d15c67` — _feat(sidebar): fold the project scope into the search row (#11315)_             | `d4a33457`  |
| 21           | New project inside the scope menu       | `d1d15c67` — _feat(sidebar): fold the project scope into the search row (#11315)_             | `d4a33457`  |
