...
> **2026-09-18**, onto `pingdotgg/t3code` `main` at **`1455cb5c`** —
> _fix(server): checkpoint workspaces with empty nested repositories (#12181)_. Took in **84
> upstream commits** (`7a368fe7..1455cb5c`). The previous tip `63f1b1a4` is backed up at
> `origin/backup/main-pre-rebase-2026-09-18`; `main` was then force-pushed to the rebased tip.
>
> **Ten commits replayed, nothing superseded, one entry re-derived.** Every drop-check in
> section 3 still comes back **0** against clean upstream. Two commits conflicted, in three
> files:
>
> - **Entry 14** — `release.yml`, which the fork deletes, took upstream's `3060cc46`
>   (_restrict stable channels to main_). Deleted again. That commit also grew
>   `.github/scripts/check-nightly-release.cjs` and its test, both kept byte-identical as before;
>   the `ci.yml` step that runs the test stays dropped. No other workflow changed this range and
>   `scripts/build-desktop-artifact.ts` is untouched, so `desktop-artifacts.yml` needed nothing.
> - **Entry 19** — upstream's `675869d2` (#12304, _preserve saved totals after transcript
>   cleanup_) rebuilt the two ends of the usage scan the fork threads its instance fields
>   through: `resolveTranscriptDirs` now takes the retention cutoff and resolves each source's
>   `volumeId` itself through a persisted per-source cache, and the aggregation loop folds
>   retained cache records into a source after its transcripts are cleaned up, keys Codex
>   records per rollout occurrence, and reports `missing` only when nothing was retained.
>   `d4d5d12e` (#10315) reorders `claimSources` by `readAt`. Took upstream's structure for all
>   of it and re-threaded `instanceId` / `displayName` / `accentColor`; details in entry 19.
>
> Every other entry replayed clean — verified per file by comparing the fork's diff against its
> base before and after the rebase, not inferred from the absence of conflicts. Two upstream
> changes widen what fork entries already do without touching them: `0150c6a5` (#12179,
> multi-model threads in separate worktrees) adds a client call site that mints a placeholder
> branch and lands in the `ws.ts` block entry 17 rewrites, and `eff44be4` (#12115) fills
> `usageLimits` for OpenCode Go, Cursor and Grok, so entry 22's picker and bubble rows now
> appear for those instances too. Both entries' notes were updated.
>
...
> client-runtime, web, server, desktop and mobile. `vp lint` over the 66 fork-changed TypeScript
> files: **0 errors**; 156 warnings, of which 154 are on upstream lines and two are unused `_`
> mock parameters in the fork's reactor test that copy the pattern of upstream's own mocks in
> the same file. `vp fmt --check` and `vp run knip:check` pass. The fork-touched test files
> pass: server usage, reactor, registry and probe suites (163), `server.test.ts` filtered to its
> worktree and bootstrap cases (17, including the branch-prefix one), shared (63),
> client-runtime (2) and the web unit files (353). Nothing was checked in a browser.
>
...
- **Checked at `1455cb5c`: keep, clean replay.** Drop-check empty. `ChatView.tsx` took ~660
  lines of upstream change this range (multi-model worktrees `0150c6a5` #12179, scroll pill
  `fcfd9f91` #12317) and `MessagesTimeline.tsx` ~690 (thought grouping `592021f0` #12302,
  `fbc8b960` #12270, `22539f2a` #12147), none of it near the fork's hunks; every touched file's
  fork diff is byte-identical before and after the replay.
...
- **Checked at `1455cb5c`: keep, clean replay.** Drop-check **0**. `uiStateStore.ts` is
  unchanged upstream this range; `ThreadStatusIndicators.tsx` took a ~175-line upstream rework
  (`01eb4b9a` #12202 notification icon alignment) that left the acknowledgement seam and the
  fork's diff untouched.
...
- **Checked at `1455cb5c`: keep, clean replay.** `AGENTS.md` is unchanged upstream this range.
...
- **Checked at `1455cb5c`: keep, refreshed.** Rebase marker moved to `1455cb5c`. One banner
  sentence re-derived: the allowances entry 22 surfaces are no longer only Claude's and
  Codex's (upstream's `eff44be4` #12115 added OpenCode Go, Cursor and Grok), so the banner
  now says "the subscription allowances upstream reports on its Limits view" without naming
  providers. Upstream's own `README.md` edit this range merged below the banner.
...
- **Checked at `1455cb5c`: keep, clean replay.** Upstream's only workflow change this range
  was `3060cc46` (_fix(release): restrict stable channels to main_) to `release.yml`, which the
  fork deletes; resolved by deleting it again. The same commit grew
  `.github/scripts/check-nightly-release.cjs` and its test — kept byte-identical, their `ci.yml`
  step stays dropped. `ci.yml` itself and `scripts/build-desktop-artifact.ts` are unchanged
  upstream, so `desktop-artifacts.yml` needed no re-derive. `grep -rn blacksmith
...
- **Checked at `1455cb5c`: keep, clean replay.** Both server drop-checks still come back **0**.
  `ClaudeProvider.ts`, `ClaudeHome.ts`, `providerSnapshot.ts` and `ProviderInstanceCard.tsx`
  are all unchanged upstream this range.
...
- **Checked at `1455cb5c`: keep, clean replay.** Still the lone hardcoded const upstream.
  `ws.ts` took `0150c6a5`'s (#12179) `requireWorktree` handling above the fork's prefix rewrite
  and interruptibility changes below it; the rewrite still sits directly above
  `gitWorkflow.createWorktree`, feeding `newRefName: worktreeBranch`, and the fork's diff is
  byte-identical to before. `ProviderCommandReactor.ts` took a 16-line upstream change away from
  the rename site; `settingsSearch.ts` a ~70-line one (`71d12d8c` #12175 searchable keybindings)
  around the fork's search item. The `server.test.ts` case replayed clean between upstream's
  fallback tests. Since the 2026-09-12 rebase the reactor
...
- **Checked at `1455cb5c`: keep, clean replay.** Upstream's `handleNewThreadClick` still never
  looks at the scope; the only upstream hits for `scopedProjectGroup` are the scope combobox.
  `Sidebar.tsx` took a two-line upstream change this range and `SidebarThreadHeader.tsx` none.
  `Sidebar.tsx` net diff is still the handler, the `scopedNewThreadProjectRef` memo and three
  header props.
...
- **Checked at `1455cb5c`: keep, server half re-derived again.** Drop-check **0**;
  `UsageProviderKind` is still `claude | codex | grok`. Upstream's `675869d2` (#12304,
  _preserve saved totals after transcript cleanup_) reworked both places in `UsageService.ts`
  the fork threads its fields through. `resolveTranscriptDirs` now takes `retentionCutoffMs`
  and resolves each source's `volumeId` itself, through a `sources` map persisted in the scan
  cache so a recreated directory keeps its fingerprint; `collectDirs` no longer reads the
  volume id. The aggregation loop no longer `continue`s on a missing directory: it folds
  retained cache records under each source's directory into that source, tags Codex records
  with a per-rollout occurrence `dedupeKey`, and reports `missing` only when nothing was
  retained. Taken wholesale; the fork's `UsageTranscriptSource` gained `volumeId`, the
  `instance` spread now sits on the single `sources.push` at the end of the loop, and
  `aggregator.add(usageRecord, instanceId)` takes upstream's re-keyed record. The fork's
  `Effect.provideService(FileSystem.FileSystem, …)` on the resolver call was dropped — the
  resolver reads the closure's `fileSystem` and never required the service. `d4d5d12e`
  (#10315) reorders `claimSources` by `readAt` (newest summary claims a fingerprint,
  environment id breaks ties); the fork's per-instance ownership sits on top of it unchanged —
  that conflict was a doc comment. `UsageService.test.ts` took ~115 lines of upstream tests
  (retention across cleanup) that merged clean around the fork's attribution case, and the
  whole suite passes. Client side, `d4d5d12e` also touched `serverUsage.test.ts` and
  `eff44be4` (#12115) `docs/user/usage.md`, both clean.
...
- **Checked at `1455cb5c`: keep, clean replay.** `scripts/build-desktop-artifact.ts` is
  unchanged upstream this range, so `resolveGitHubPublishConfig` still reads
  `T3CODE_DESKTOP_UPDATE_REPOSITORY` first and still requires `owner/repo`; the env line is
  untouched in `desktop-artifacts.yml`. `SidebarChrome.tsx` took a ten-line upstream change
  away from the GitHub item.
...
- **Checked at `1455cb5c`: keep, clean replay.** Both drop-checks come back **0**. `usedPercent`
  still crosses the wire (`packages/contracts/src/providerUsageLimits.ts`). `ChatComposer.tsx`
  took ~170 lines of upstream change this range (`0150c6a5` #12179 multi-model, `f4ef5155`
  #12075 send shortcut) and `ModelPickerContent.tsx` ~65, neither touching the fork's hunks;
  fork diffs byte-identical before and after. `eff44be4` (#12115) widened the reach of this
  entry to three more providers, as decision 3 now records.
...
| ------------ | --------------------------------------- | --------------------------------------------------------------------------------------------- | ----------- |
| 1            | Windows build: no shell mode            | `edb1240` — _fix(cli): publish nightly branded favicons (#4372)_                              | `1455cb5c`  |
| 4            | Terminal Ctrl-chord forwarding          | `acf761b2` — _feat(web): render terminals with libghostty-vt (#4860)_                         | `1455cb5c`  |
| 5 (core)     | Thread-scoped changed files             | `AssistantChangedFilesSection` per-turn checkpoints                                           | `1455cb5c`  |
| 7            | Shell-style composer recall             | `fd773172` — _feat(web): recall sent prompts with the up arrow (#9173)_                       | `1455cb5c`  |
| 8            | Full timestamp on hover                 | `formatChatTimestampTooltip` in `apps/web/src/timestampFormat.ts`                             | `1455cb5c`  |
| 9            | Always-visible new-thread btn           | `0de95407` — _feat: sidebar v2 is now the default sidebar (#5672)_                            | `1455cb5c`  |
| 10           | Package-local vitest configs            | `vp` (vite-plus) test-runner migration                                                        | `1455cb5c`  |
| 12 (symlink) | `CLAUDE.md` symlink → `AGENTS.md`       | `4cb676cc` — _docs: point CLAUDE.md at AGENTS.md with an @import (#7171)_                     | `1455cb5c`  |
| 15 (banner)  | Status banner prefers server msg        | `06336460` — _feat(providers): add Google Antigravity via the official ACP agent (#9348)_     | `1455cb5c`  |
| 16           | Usage scans every provider instance     | `2db675ae` — _fix(usage): respect provider account homes (#11485)_                            | `1455cb5c`  |
| 22 (server)  | Subscription usage collection           | `19d8ab2a` — _feat(usage): show Codex and Claude subscription limits on a Limits tab (#9507)_ | `1455cb5c`  |
| 18 (layout)  | Labelled new-thread btn under scope row | `d1d15c67` — _feat(sidebar): fold the project scope into the search row (#11315)_             | `1455cb5c`  |
| 21           | New project inside the scope menu       | `d1d15c67` — _feat(sidebar): fold the project scope into the search row (#11315)_             | `1455cb5c`  |
