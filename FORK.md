...
> **2026-09-19**, onto `pingdotgg/t3code` `main` at **`8c3b5bef`** —
> _fix(mobile): prefer pull-to-refresh on list screens (#12515)_. Took in **65 upstream
> commits** (`1455cb5c..8c3b5bef`). The previous tip `34fd7c9f` is backed up at
> `origin/backup/main-pre-rebase-2026-09-19`; `main` was then force-pushed to the rebased tip.
>
> **Ten commits replayed, nothing superseded, nothing re-derived.** Every drop-check in
> section 3 still comes back **0** against clean upstream. Three commits conflicted, in seven
> files, all resolved mechanically:
>
> - **Entry 14** — four modify/delete conflicts. Upstream's `74796256` (#12401, _deploy with
>   the Alchemy CLI and publish client config through an Action_) rewrote `deploy-relay.yml` and
>   the tracing-config read in `release.yml`, both of which the fork deletes: deleted again. The
>   same commit deleted `infra/relay/scripts/deploy.test.ts` itself, taking the fork's fallout
>   edit to that file with it. `d547e3b1` (#12326, Effect rc.115) touched
>   `scripts/smoke-cli-archive.ts`, still referenced only by the deleted `release-desktop.yml`:
>   deleted again.
> - **Entry 19** — `ea15f17a` (#12512) dropped the `card` prop from mobile's `SettingsSection`
>   at the three call sites in `UsageRouteScreen.tsx`, two of which the fork rewrites. Kept the
>   fork's per-instance sections without the prop.
> - **Entry 5** — `9051ed9c` (#11842, _open diff files from a right-click context menu_) added an
>   `onFileContextMenu` prop to `ChangedFilesCard` on the lines beside the fork's
>   `onCommitTurnFiles`, in the prop type, the destructure and the `MessagesTimeline.tsx` call
>   site. Kept both props everywhere.
>
> Every other entry replayed clean — verified per file by comparing the added and removed lines
> of each fork commit before and after the replay: identical for every file except the four
> entry 14 deletions above, which now delete upstream's newer copies. The one upstream change
> worth reading for its effect on a fork entry is the Effect rc.115 upgrade (`d547e3b1`), which
> renamed the `Config` / `Flag` constructors in `scripts/build-desktop-artifact.ts`; no flag,
> env or job shape changed, so `desktop-artifacts.yml` (entries 14 and 20) needed nothing.
>
...
> files: **0 errors** (eight `set-state-in-effect` warnings, all on upstream lines of
> `ChatView.tsx`). `vp fmt --check` and `vp run knip:check` pass. The fork-touched test files
> pass: server usage, reactor, registry and probe suites (163), `server.test.ts` filtered to its
> worktree, bootstrap and prefix cases (17), shared (63), client-runtime (2) and the web unit
> files (312). Nothing was checked in a browser.
>
...
- **Checked at `8c3b5bef`: keep, one conflict resolved.** Drop-check empty. Upstream's
  `9051ed9c` (#11842, right-click context menu on changed files) added an `onFileContextMenu`
  prop to `ChangedFilesCard` on the lines directly beside the fork's `onCommitTurnFiles` — the
  prop type, the destructure and the `MessagesTimeline.tsx` call site all conflicted, each
  resolved by keeping both props. `ChatView.tsx` took a two-line upstream change (`3fd5d643`
  #12315) away from the fork's hunks. The fork's added and removed lines are identical before
...
- **Checked at `8c3b5bef`: keep, clean replay.** Drop-check **0**. `uiStateStore.ts`,
  `Sidebar.logic.ts` and `ThreadStatusIndicators.tsx` took nothing near the acknowledgement seam
  upstream this range (`Sidebar.logic.ts` changed for search only, `c557bb10` #11761);
  `ChatView.tsx` took two upstream lines away from the stamp.
...
- **Checked at `8c3b5bef`: keep, clean replay.** `AGENTS.md` is unchanged upstream this range.
...
- **Checked at `8c3b5bef`: keep, refreshed.** Rebase marker moved to `8c3b5bef`; no entry
  moved this rebase, so the banner text is otherwise unchanged. Upstream did not touch
  `README.md` this range.
...
- **Checked at `8c3b5bef`: keep, four deletions re-applied.** Upstream's `74796256` (#12401,
  _deploy with the Alchemy CLI and publish client config through an Action_) rewrote the deploy
  step in `deploy-relay.yml` and the tracing-config read in `release.yml` — both files the fork
  deletes, both deleted again; it also deleted `infra/relay/scripts/deploy.test.ts` itself, so
  the fork's fallout edit to that file is gone with nothing to restore, and it reworded the
  relay README around the fork note, which merged clean. `d547e3b1` (#12326, Effect rc.115)
  renamed `Flag.string` → `Flag.String` in `scripts/smoke-cli-archive.ts`, still referenced
  only by the deleted `release-desktop.yml`: deleted again, `knip:check` passes. `8ebb6112`
  (#12492) changed `.github/scripts/thread-transfer-report.cjs` and its test, kept
  byte-identical; `thread-transfer-report.yml`, `ci.yml` and `release-desktop.yml` are
  unchanged upstream. `scripts/build-desktop-artifact.ts` took the same Effect API renames
  (`Config.String`, `Flag.Boolean`, `Flag.Literals`) plus an asar glob `cwd` fix (`52e4b442`
...
- **Checked at `8c3b5bef`: keep, clean replay.** Both server drop-checks still come back **0**.
  `packages/contracts/src/server.ts` took four upstream lines (`3bb06ad9` #12493, OTLP log
  export) away from `ServerProviderConfigDirectory`; `ClaudeProvider.ts`, `ClaudeHome.ts`,
  `providerSnapshot.ts`, `providerStatusCache.ts` and `ProviderInstanceCard.tsx` are unchanged
  upstream this range. `claudeSignedOutMessage` is still used only at turn time in
  `ClaudeAdapter.ts`, not in the status probe.
...
- **Checked at `8c3b5bef`: keep, clean replay.** Still the lone hardcoded const upstream.
  `ws.ts` took eight upstream lines (`3bb06ad9` #12493, `e84f23a9` #11825) away from the
  `prepareWorktree` block; `settings.ts` and `server.test.ts` took small changes from the same
  OTLP commit and the Effect upgrade (`d547e3b1` #12326), all merging clean around the fork's
  key and test case. `git.ts`, the reactor, `settingsSearch.ts` and the settings UI are
  unchanged upstream.
...
- **Checked at `8c3b5bef`: keep, clean replay.** Upstream's `handleNewThreadClick` still never
  looks at the scope; its `scopedProjectGroup` hits are the scope combobox, the search-empty
  state and a new member-set memo for search. `Sidebar.tsx` took ~100 upstream lines
  (`c557bb10` #11761 search matches message content, `256e630e` #8719 filter sidebar from the
  thread menu) and `Sidebar.logic.ts` / its test ~45 and ~40 (`c557bb10`), all clear of the
  fork's handler, memo, helper and props. `SidebarThreadHeader.tsx` is unchanged upstream.
...
- **Checked at `8c3b5bef`: keep, one conflict resolved.** Drop-check **0**;
  `UsageProviderKind` is still `claude | codex | grok`. Nothing on the server side changed
  upstream this range (`usage.ts`, `UsageService.ts`, `usageAggregation.ts`, `usageMerge.ts`,
  `usageFormat.ts` untouched), nor the web usage components or any test fixture. Mobile:
  `ea15f17a` (#12512, native settings and snooze controls) dropped the `card` prop from
  `SettingsSection` at the three call sites in `UsageRouteScreen.tsx`, two of which the fork
  rewrites; resolved by keeping the fork's per-instance sections without the prop.
...
- **Checked at `8c3b5bef`: keep, clean replay.** `scripts/build-desktop-artifact.ts` changed
  upstream only for the Effect rc.115 constructor renames (`Config.string` → `Config.String`,
  `d547e3b1` #12326) and an asar glob fix; `resolveGitHubPublishConfig` still reads
  `T3CODE_DESKTOP_UPDATE_REPOSITORY` first and still requires `owner/repo`. `SidebarChrome.tsx`,
  `branding.ts`, `vite.config.ts` and `vite-env.d.ts` are unchanged upstream this range.
...
- **Checked at `8c3b5bef`: keep, clean replay.** Both drop-checks come back **0**. None of the
  entry's files changed upstream this range (`ChatComposer.tsx`, `ModelPickerContent.tsx`,
  `ContextWindowMeter.tsx` untouched); `usedPercent` still crosses the wire.
