...
(5, 6, 18). Everything else is byte-identical to upstream — `native/`, `scripts/`,
...
> **2026-09-13**, onto `pingdotgg/t3code` `main` at **`8ddd9f7e`** —
> _fix(desktop): bound backend shutdown wait during quit (#7599)_. Took in **27 upstream
> commits** (`cf1ba3d7..8ddd9f7e`). The previous tip `b35f21e5` is backed up at
> `origin/backup/main-pre-rebase-2026-09-13`; `main` was then force-pushed to the rebased tip.
>
> **Ten commits replayed, one supersession, one entry narrowed.** Upstream's `d1d15c67f`
> (#11315, _fold the project scope into the search row_) rebuilt the sidebar header as one row —
> search spanning it, then a segmented icon well of project scope, new project and new thread,
> extracted into `sidebar/SidebarThreadHeader.tsx`. That is upstream's answer to the layout
> complaints behind entries 18 and 21, so **entry 21 moved to section 4** and **entry 18 was
> narrowed to its behaviour half**: the scope-aware new thread target, re-derived onto the new
> header (details in entry 18). Every other drop-check in section 3 still comes back **0**.
>
> **Three entries conflicted, in three files**, and only the sidebar one was a real rework:
>
> - **Entry 17** — the `@t3tools/shared/git` import line in `ProviderCommandReactor.ts` sat next
>   to upstream's new `composerContextReferences` import (`348645152` #11363). Both kept.
> - **Entry 5** — `MessagesTimeline.tsx` declares `onCommitTurnFiles` right under
>   `onRevertToTurnCount`, whose signature upstream's rewind work (`efccda9ac` #11358) changed.
>   Took upstream's signature, kept the fork's prop.
> - **Entries 18 + 21** — the header block, as above. Resolved by taking upstream's
>   `SidebarThreadHeader` wholesale, deleting the fork's in-menu "New project" button and its
>   handler, and passing the scoped tooltip through a new optional `newThreadLabel` prop on the
>   header (one fork-owned prop in an upstream file). `Sidebar.logic.ts`'s
>   `resolveNewThreadClickTarget` and its tests replayed clean. The entry's commit was reworded
>   to match its narrowed scope.
>
> **Entry 14: nothing to do.** `.github/` is byte-identical between `cf1ba3d7` and `8ddd9f7e`.
>
...
> files: **0 errors**. `vp fmt --check` passes. `vp run knip:check` passes. The fork-touched
> test files pass except the known `server.test.ts` root-uid stat case (the container runs as
> root, so `chmod 0o000` cannot make a stat fail; same artifact as every rebase since
> 2026-08-28). Nothing was checked in a browser; the re-derived header is the one thing worth
> a look (entry 18).
>
...
> section 4 or 5. Numbers 1, 4, 7, 8, 9, 10, 21, the symlink half of 12, the banner half of 15, the
> layout half of 18 and the server half of 22 are superseded; 2, 3 and 11 are dropped.
...
- **Checked at `8ddd9f7e`: keep.** Drop-check empty. One adjacent-line conflict in
  `MessagesTimeline.tsx`: the fork declares `onCommitTurnFiles` right under
  `onRevertToTurnCount`, whose signature upstream's rewind work (`efccda9ac` #11358) extended
  with a `messageId`. Upstream's signature taken, fork prop kept. The `ChatView.tsx` gate from
  the previous rebase (`paintOnlyDisplayedTimeline || !isServerThread ? undefined : …`) still
  holds and replayed clean, as did `GitActionsControl.tsx`.
...
- **Checked at `8ddd9f7e`: keep, clean replay.** Drop-check **0**. `uiStateStore.ts` and
  `ThreadStatusIndicators.tsx` are unchanged upstream this range; upstream's status-icon pass
  (`03e135577` #11461) did not touch the acknowledgement seam.
...
- **Checked at `8ddd9f7e`: keep, clean replay.** Upstream did not touch `AGENTS.md` this range.
...
- **Checked at `8ddd9f7e`: keep, refreshed.** Rebase marker moved to `8ddd9f7e` and the banner
  text re-derived: entry 21 moved out, so it now lists three web UX changes and describes the
  new thread button as scope-following rather than "under the project selector".
...
- **Checked at `8ddd9f7e`: keep; `.github/` untouched upstream this range.** No workflow,
  action or script changed between `cf1ba3d7` and `8ddd9f7e`, so nothing was re-derived and
...
- **Checked at `8ddd9f7e`: keep, clean replay.** Both server drop-checks still come back **0**.
  `ClaudeProvider.ts`, `ClaudeHome.ts`, `providerSnapshot.ts` and `ProviderInstanceCard.tsx` are
  all unchanged upstream this range.
...
- **Checked at `8ddd9f7e`: keep, clean replay.** Drop-check **0**. `UsageService.ts` and
...
- **Checked at `8ddd9f7e`: keep.** Still the lone hardcoded const upstream. One import-line
  conflict in `ProviderCommandReactor.ts`: the fork's `buildWorktreeBranchName` import from
  `@t3tools/shared/git` sits beside upstream's new `composerContextReferences` import
  (`348645152` #11363); both kept, nothing else moved. Since the 2026-09-12 rebase the reactor
...
- **Checked at `8ddd9f7e`: keep, narrowed and re-derived.** Upstream's `d1d15c67f` (#11315)
  folded the project scope into the search row: search spans, then a segmented icon well of scope,
  new project and new thread, all in a new `SidebarThreadHeader` component. Its
  `handleNewThreadClick` still calls `shouldCreateNewThreadInCurrentProject` and never looks at
  the scope, so the behaviour half survives. The layout half is gone (section 4). `Sidebar.tsx`
  net diff is now the handler, the `scopedNewThreadProjectRef` memo and three header props.
...
- **Checked at `8ddd9f7e`: keep, clean replay.** Drop-check **0**; `UsageProviderKind` is
  still `claude | codex | grok`. The only usage change upstream this range is `fcbe45796`
  (#10601), which touches both `UsageLimitsPooled.tsx` files (Limits view, provider-kind
  colours) and nothing the fork keys by instance.
...
- **Checked at `8ddd9f7e`: keep, clean replay.** `resolveGitHubPublishConfig` and its
  `T3CODE_DESKTOP_UPDATE_REPOSITORY` read are unchanged upstream, so the disable holds.
  `SidebarChrome.tsx` took a one-line upstream change (`b0c6c3b2f` footer spacing) without
  conflicting.
...
- **Checked at `8ddd9f7e`: keep, clean replay.** Both drop-checks come back **0**. `usedPercent`
  still crosses the wire. `ChatComposer.tsx` grew by ~1,000 lines upstream this range
  (`4fed6cfb3` #11265 inline file previews and attachment chips) without touching the five
  hunks; verified by grep after the replay, not assumed from the clean merge.
...
| ------------ | --------------------------------------- | --------------------------------------------------------------------------------------------- | ----------- |
| 1            | Windows build: no shell mode            | `edb1240` — _fix(cli): publish nightly branded favicons (#4372)_                              | `8ddd9f7e`  |
| 4            | Terminal Ctrl-chord forwarding          | `acf761b2` — _feat(web): render terminals with libghostty-vt (#4860)_                         | `8ddd9f7e`  |
| 5 (core)     | Thread-scoped changed files             | `AssistantChangedFilesSection` per-turn checkpoints                                           | `8ddd9f7e`  |
| 7            | Shell-style composer recall             | `fd773172` — _feat(web): recall sent prompts with the up arrow (#9173)_                       | `8ddd9f7e`  |
| 8            | Full timestamp on hover                 | `formatChatTimestampTooltip` in `apps/web/src/timestampFormat.ts`                             | `8ddd9f7e`  |
| 9            | Always-visible new-thread btn           | `0de95407` — _feat: sidebar v2 is now the default sidebar (#5672)_                            | `8ddd9f7e`  |
| 10           | Package-local vitest configs            | `vp` (vite-plus) test-runner migration                                                        | `8ddd9f7e`  |
| 12 (symlink) | `CLAUDE.md` symlink → `AGENTS.md`       | `4cb676cc` — _docs: point CLAUDE.md at AGENTS.md with an @import (#7171)_                     | `8ddd9f7e`  |
| 15 (banner)  | Status banner prefers server msg        | `06336460` — _feat(providers): add Google Antigravity via the official ACP agent (#9348)_     | `8ddd9f7e`  |
| 22 (server)  | Subscription usage collection           | `19d8ab2a` — _feat(usage): show Codex and Claude subscription limits on a Limits tab (#9507)_ | `8ddd9f7e`  |
| 18 (layout)  | Labelled new-thread btn under scope row | `d1d15c67` — _feat(sidebar): fold the project scope into the search row (#11315)_             | `8ddd9f7e`  |
| 21           | New project inside the scope menu       | `d1d15c67` — _feat(sidebar): fold the project scope into the search row (#11315)_             | `8ddd9f7e`  |
