...
> **2026-09-10**, onto `pingdotgg/t3code` `main` at **`b7b3ef1e`** —
> _fix(ui): use available space for composer model names (#11002)_. Took in **153 upstream
> commits** (`5b0c923e..b7b3ef1e`). The previous tip `a98e086b` is backed up at
> `origin/backup/main-pre-rebase-2026-09-10`; `main` was then force-pushed to the rebased tip.
>
...
> replayed in order and `git log upstream/main..main` still reads as section 3 does. Keep it that
> way: when an entry changes, amend its commit at the next rebase rather than stacking fix-ups.
>
> **Five conflicts, none substantive** — every one an import list or an adjacent-line hunk:
>
> - `release.yml`, `deploy-relay.yml` — modify/delete (entry 14), resolved as delete. Upstream
>   added a `deploy_marketing` job (Vercel secrets, Blacksmith) to the former and an
>   `FCM_SERVICE_ACCOUNT` secret to the latter; both still fail the entry 14 rule.
> - `git.test.ts` — upstream (`33242d016` #11007) added `parseOriginUrlFromGitConfig` to the
>   import list beside entry 17's `DEFAULT_WORKTREE_BRANCH_PREFIX`.
> - `GitActionsControl.tsx` — upstream (`3faeee49a` #10832) replaced the `AnimatedHeight` import
>   with the shared wizard components; entry 5's `deriveCommitExcludedFilePaths` import sits
>   beside it. The 704-line upstream rework left the `commitPreselection` effect untouched.
> - `Sidebar.tsx` — import list only. Upstream (`f0401c629` #10870) renamed
>   `searchSidebarThreadsByTitle` to `searchSidebarThreads`; the first resolution kept the **old**
>   name and only `tsc` caught it. **Trap for next time:** in a rebase, `HEAD` is upstream and the
>   commit being replayed is the fork — read the markers that way round before picking a side.
> - `ChatComposer.tsx` — upstream (`de545c417` #10727, `7fbc545ae` #10768) added
>   `reserveContextWindowMeter` and a `ContextWindowMeterPlaceholder` on the lines entry 22's
>   `activeSubscriptionUsage` prop and read occupy; both kept.
>
> **Entry 14: `ci.yml` took one upstream change, kept.** The `rust` job now formats and tests
> three crates (`resource-monitor`, `kde-snap-shot`, `hyprland-snap-shot`) in a loop; it runs on a
> plain runner with no extra system packages, so it merged as-is. `release.yml`'s build job added
> a cache for the two Linux capture helpers and a `T3CODE_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS`
> flag (`299404a75` #8103); **nothing to port** — `desktop-artifacts.yml` never caches native
> builds, always sets up Rust, and `build-desktop-artifact.ts` cargo-builds the helpers itself
> when the flag is unset. Its Linux apt step is unchanged upstream. No new upstream workflows.
>
...
> client-runtime, web, server, desktop and mobile (the Effect language-service `suggestion`
> lines are not errors; filter on `error TS`). `vp lint` over the 66 fork-changed TypeScript
> files: **0 errors**, warnings only, all pre-existing patterns. `vp fmt --check` passes.
> `vp run knip:check` passes. The 18 fork-touched test files: **696 passed, 1 failed** — the
...
- **Checked at `b7b3ef1e`: keep.** Drop-check empty. Upstream reworked 704 lines of
  `GitActionsControl.tsx` (shared setup wizards, `3faeee49a`); only the import list conflicted and
  the `commitPreselection` effect replayed clean.
...
- **Checked at `b7b3ef1e`: keep, clean replay.** Drop-check **0**. Upstream touched
  `uiStateStore.ts`, `ThreadStatusIndicators.tsx` and `ChatView.tsx` this range without
  conflicting.
...
- **Checked at `b7b3ef1e`: keep, clean replay.** Upstream did not touch `AGENTS.md` this range.
...
- **Checked at `b7b3ef1e`: keep, refreshed.** Rebase marker moved to `b7b3ef1e`; the banner text
...
- **Checked at `b7b3ef1e`: keep; no new upstream workflows.** `ci.yml` took upstream's
  three-crate `rust` job loop unchanged. `release.yml` added a Linux capture-helper cache and a
  Vercel marketing deploy; neither needs porting (see section 2). `deploy-relay.yml` grew an FCM
  secret and stays deleted. `grep -rn blacksmith .github/workflows/` still matches only the
  explanatory comment in `desktop-artifacts.yml`.
...
- **Checked at `b7b3ef1e`: keep, clean replay.** Both server drop-checks still come back **0**.
  Upstream changed one line of `ClaudeProvider.ts` this range (`3bf74eb6d` #10549, usage limits on
  retried turns) and nothing in `ProviderInstanceCard.tsx`.
...
- **Checked at `b7b3ef1e`: keep, clean replay.** Drop-check **0**. `UsageService.ts` and
...
- **Checked at `b7b3ef1e`: keep.** Still the lone hardcoded const upstream. Only the
  `git.test.ts` import list conflicted (upstream's new `parseOriginUrlFromGitConfig`); the
  `ProviderCommandReactor.ts` and `ws.ts` hunks replayed clean.
...
- **Checked at `b7b3ef1e`: keep.** Upstream changed **724 lines** of `Sidebar.tsx` this range
  (file drops onto threads, PR search, favicon plumbing). Only the import list conflicted — the
  `searchSidebarThreads` rename noted in section 2 — and the header JSX replayed clean. **Still
  the fork's highest-churn UI surface** — re-derive rather than replay whenever the block itself
  moves.
...
- **Checked at `b7b3ef1e`: keep, clean replay.** Drop-check **0**; `UsageProviderKind` is still
  `claude | codex | grok`. Upstream's usage changes this range (`1f14d6d10` account column
  alignment, `6ba15c027` hiding email-bearing account labels, `6abdf37a5` hub reset credits) stay
  on the Limits side and added no fixtures needing the instance fields; typecheck is clean in every
  package.
...
- **Checked at `b7b3ef1e`: keep, clean replay.** `resolveGitHubPublishConfig` and its
...
- **Checked at `b7b3ef1e`: keep, clean replay.** Upstream still renders the standalone
...
- **Checked at `b7b3ef1e`: keep.** Both drop-checks come back **0**. `usedPercent` still crosses
  the wire (`packages/contracts/src/providerUsageLimits.ts`). Upstream's composer-footer
  placeholder work (`de545c417`, `7fbc545ae`) landed on the same lines as two of the five
  `ChatComposer.tsx` hunks; the meter now renders inside a
  `activeContextWindow ? <ContextWindowMeter …/> : reserve ? <Placeholder/> : null` chain with the
  `subscriptionUsage` prop untouched.
...
| ------------ | --------------------------------- | --------------------------------------------------------------------------------------------- | ----------- |
| 1            | Windows build: no shell mode      | `edb1240` — _fix(cli): publish nightly branded favicons (#4372)_                              | `b7b3ef1e`  |
| 4            | Terminal Ctrl-chord forwarding    | `acf761b2` — _feat(web): render terminals with libghostty-vt (#4860)_                         | `b7b3ef1e`  |
| 5 (core)     | Thread-scoped changed files       | `AssistantChangedFilesSection` per-turn checkpoints                                           | `b7b3ef1e`  |
| 7            | Shell-style composer recall       | `fd773172` — _feat(web): recall sent prompts with the up arrow (#9173)_                       | `b7b3ef1e`  |
| 8            | Full timestamp on hover           | `formatChatTimestampTooltip` in `apps/web/src/timestampFormat.ts`                             | `b7b3ef1e`  |
| 9            | Always-visible new-thread btn     | `0de95407` — _feat: sidebar v2 is now the default sidebar (#5672)_                            | `b7b3ef1e`  |
| 10           | Package-local vitest configs      | `vp` (vite-plus) test-runner migration                                                        | `b7b3ef1e`  |
| 12 (symlink) | `CLAUDE.md` symlink → `AGENTS.md` | `4cb676cc` — _docs: point CLAUDE.md at AGENTS.md with an @import (#7171)_                     | `b7b3ef1e`  |
| 15 (banner)  | Status banner prefers server msg  | `06336460` — _feat(providers): add Google Antigravity via the official ACP agent (#9348)_     | `b7b3ef1e`  |
| 22 (server)  | Subscription usage collection     | `19d8ab2a` — _feat(usage): show Codex and Claude subscription limits on a Limits tab (#9507)_ | `b7b3ef1e`  |
